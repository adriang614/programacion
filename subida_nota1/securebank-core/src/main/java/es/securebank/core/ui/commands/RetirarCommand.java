...
public class RetirarCommand implements Command {
    @Override public String getNombre() { return "retirar"; }
    @Override public String getDescripcion() { return "Retira dinero de una cuenta"; }
    @Override public String getUso() { return "retirar <iban> <importe>"; }
    ...
    public void execute(String[] args, AppContext context) throws CommandException {
        ...
        try {
            context.getAntiFraudeService().comprobarRetiro(clienteId, iban, importe);
            context.getTransaccionService().retirar(iban, importe, "Retiro manual");
        } catch (FraudeConcurrentException e) {
            throw new CommandException("ALERTA FRAUDE: Cuenta bloqueada por actividad sospechosa.");
        }
    }
}
